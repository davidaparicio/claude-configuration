...
`animate` is the single animation skill. Its focused workflows and long-form lessons live as internal references under `references/`; they are not separate skills or commands.

Select the smallest relevant mode below. Read its linked `guide.md` completely before acting, then load only the adjacent recipes, standards, snippets, or templates that the guide explicitly requires. Resolve relative links from the guide's own directory.

| Route | Internal mode guide |
| --- | --- |
| CSS transitions, keyframes, transforms, hover, press, clip-path | [CSS animations](references/css-animations/guide.md) |
| Motion for React / Framer Motion | [Motion for React](references/motion-react/guide.md) |
| Strict review with approve/block verdict | [Review animations](references/review-animations/guide.md) |
| Read-only animation audit and implementation plan | [Improve animations](references/improve-animations/guide.md) |
| Find the few UI areas that should animate | [Find opportunities](references/find-animation-opportunities/guide.md) |
| Choose an animation or UI library | [Pick a library](references/pick-ui-library/guide.md) |
| Interview the user and produce a motion brief | [Motion brief](references/motion-brief/guide.md) |
| Name an animation effect | [Animation vocabulary](references/animation-vocabulary/guide.md) |
| Diagnose jank, frame budget, GPU, or rendering cost | [Animation performance](references/animation-performance/guide.md) |
| Reduced-motion and motion accessibility | [Animation accessibility](references/animation-accessibility/guide.md) |
| Build several animation/UI variants behind a picker | [Prototype variants](references/prototype/guide.md) |

If no focused mode is narrower than this core guide, use this file directly. If several apply, load them in dependency order: brief or library choice, implementation, then performance/accessibility/review.
...
For the full frame-budget model (the Layout/Paint/Composite pipeline, main-thread vs GPU, React re-renders, and a diagnosis checklist for dropped frames), load the [animation performance guide](references/animation-performance/guide.md).
...
For the two-variant workflow and the recipes for autoplaying media, looping animation, and smooth scrolling, load the [animation accessibility guide](references/animation-accessibility/guide.md).
...
For a dedicated, exhaustive reviewer with a strict block/approve verdict, load the [review mode](references/review-animations/guide.md). To name an effect, load the [vocabulary mode](references/animation-vocabulary/guide.md).
