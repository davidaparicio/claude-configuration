...
A divergence mode. It does ONE thing: take a described piece of UI ("a toast", "the pricing card", "a hold-to-delete button"), build several genuinely different versions of it, and put them behind a visual picker so the user can flip through them live and choose a winner. It does not review existing UI (use [review mode](../review-animations/guide.md)), plan fixes for it (use [improvement mode](../improve-animations/guide.md)), or choose dependencies (use [library mode](../pick-ui-library/guide.md)).
