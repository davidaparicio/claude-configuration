...
It is not a diff review (use [review mode](../review-animations/guide.md)), it is not a search for missing motion (use [opportunity mode](../find-animation-opportunities/guide.md)), and **it does not implement fixes itself**. Judgment happens here; execution happens elsewhere.
...
| `execute <plan>` | Dispatch an executor to implement the plan in an isolated worktree, then review its diff with the `animate` review-mode bar and render a verdict |
