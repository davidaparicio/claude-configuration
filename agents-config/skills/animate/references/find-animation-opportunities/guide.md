...
A scouting mode. It does ONE thing: look at UI that exists and propose the small number of places where motion would genuinely improve it — with exact values — and say plainly what should be left alone. It does not implement anything, and it does not review motion that's already there (use [review mode](../review-animations/guide.md)).
...
To turn accepted suggestions into implementation plans, use [improvement mode](../improve-animations/guide.md). To implement one directly, return to the core `animate` workflow.
