---
name: impeccable
description: Route production frontend design work. Use to design, redesign, critique, audit, polish, clarify, harden, optimize, adapt, animate, colorize, or improve websites, landing pages, dashboards, app UI, components, onboarding, and design systems. Not for backend-only tasks.
---
...
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, run `node .agents/skills/impeccable/scripts/palette.mjs` to receive a brand seed color and composition guidance. This is the anchor for your primary brand color. Compose the rest of the palette (bg, surface, ink, accent, muted) around it per the script's instructions. Use OKLCH throughout. **Skip this step only if step 3 found committed brand colors in existing tokens; in that case identity-preservation wins.** In shadcn projects, the theme tokens are the source of truth: use `primary`, `primary-foreground`, `secondary`, `muted`, `accent`, `destructive`, `success`, `warning`, and `border` tokens. Do not introduce fallback blue, sky, cyan, or indigo classes because they look like a default AI accent and may not belong to the theme.
...
- In shadcn or tokenized projects, never use hardcoded accent families such as `blue-*`, `sky-*`, `cyan-*`, or `indigo-*` as a generic UI accent. Use the existing theme `primary` token and its foreground pair for primary accents, or use semantic tokens such as `success`, `warning`, and `destructive` when the state demands it.
