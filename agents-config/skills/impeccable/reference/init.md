...
- **Design tokens / CSS variables**: Existing color palettes, font stacks, spacing scales. If the project uses shadcn tokens, treat them as the design source of truth. Capture `primary` and `primary-foreground` as the accent pair, and do not seed or recommend generic `blue-*`, `sky-*`, `cyan-*`, or `indigo-*` accent classes unless those colors are explicitly present in the committed theme.
