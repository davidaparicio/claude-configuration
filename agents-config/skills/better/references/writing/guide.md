...
How copy renders (capitalization via `text-transform`, truncation, smart punctuation) is covered by the [typography](../typography/guide.md) reference; error markup and announcements (`aria-invalid`, live regions) by the [accessibility](../accessibility/guide.md) reference; room for translated strings by the [layout](../layout/guide.md) reference.
...
Use this format only when the user asks for a standalone writing review. When `better` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
