...
Typography (text wrapping, font rendering, tabular numbers, spacing) is covered by the [typography](../typography/guide.md) reference; use that for anything text-related. Accessibility (hit areas, focus states, keyboard support, ARIA, reduced motion) is covered by the [accessibility](../accessibility/guide.md) reference. Layout structure (grouping, spacing between sections, breakpoints, spatial RTL) is covered by the [layout](../layout/guide.md) reference.
...
Use this format only when the user asks for a standalone UI-polish review. When `better` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
