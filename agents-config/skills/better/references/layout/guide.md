...
Hit-area sizes and focus behavior are covered by the [accessibility](../accessibility/guide.md) reference; visual polish (radius, shadows, animation) by the [ui](../ui/guide.md) reference; line length and text spacing by the [typography](../typography/guide.md) reference.
...
Without an established density system, start with `12px` between adjacent bordered or filled controls and `24px` of clearance around borderless text- and icon-only controls. Compact layouts may use less when [accessibility](../accessibility/guide.md) hit areas do not overlap and the controls remain visually distinct.
...
Use this format only when the user asks for a standalone layout review. When `better` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
